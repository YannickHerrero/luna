...
The main app requests system notification authorization after authenticated bootstrap. Authorized devices register their current APNs token, environment, app topic, and version through the authenticated device route; denied devices disable their server registration. Token rotation is handled by subsequent APNs callbacks and foreground activation retries transient failures. Notification taps reuse `luna://conversation/<UUID>`, while a foreground notification for the conversation already on screen is suppressed. Simulator tests cover payload validation and lifecycle decisions, but production delivery, foreground presentation, token rotation, and tap behavior still require an explicitly approved signed physical-device verification with provider credentials.
